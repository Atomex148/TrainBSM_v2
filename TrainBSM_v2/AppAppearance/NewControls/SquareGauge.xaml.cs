...
namespace TrainBSM_v2.AppAppearance.NewControls
{
    public partial class SquareGauge : UserControl, IGaugeControl
    {
        ...
        public static readonly DependencyProperty MinValueProperty = DependencyProperty.Register(
            nameof(MinValue), typeof(double), typeof(SquareGauge), new PropertyMetadata(0.0, OnRangeChanged));

        public static readonly DependencyProperty MaxValueProperty = DependencyProperty.Register(
            nameof(MaxValue), typeof(double), typeof(SquareGauge), new PropertyMetadata(100.0, OnRangeChanged));

        public static readonly DependencyProperty YellowZoneLowProperty = DependencyProperty.Register(
            nameof(YellowZoneLow), typeof(double?), typeof(SquareGauge), new PropertyMetadata(null, OnZoneChanged));

        public static readonly DependencyProperty RedZoneLowProperty = DependencyProperty.Register(
            nameof(RedZoneLow), typeof(double?), typeof(SquareGauge), new PropertyMetadata(null, OnZoneChanged));

        public static readonly DependencyProperty YellowZoneHighProperty = DependencyProperty.Register(
            nameof(YellowZoneHigh), typeof(double?), typeof(SquareGauge), new PropertyMetadata(null, OnZoneChanged));

        public static readonly DependencyProperty RedZoneHighProperty = DependencyProperty.Register(
            nameof(RedZoneHigh), typeof(double?), typeof(SquareGauge), new PropertyMetadata(null, OnZoneChanged));

        public static readonly DependencyProperty MajorTicksProperty = DependencyProperty.Register(
            nameof(MajorTicks), typeof(int), typeof(SquareGauge), new PropertyMetadata(11, OnRangeChanged));
        ...
        private const double _maxAngle = 0;
        private const double _centerX = 80;
        private const double _centerY = 80;
        private const double _tickRadius = 75;
        private const double _labelRadius = 58;
        private const double _segmentRadius = 40;
        private const double _segmentThickness = 8;
        ...
        private void _DrawTick(double angleDegrees, double length, double thickness, Brush brush)
        {
            ...
            double x1 = _centerX + _tickRadius * Math.Cos(radians);
            double y1 = _centerY + _tickRadius * Math.Sin(radians);
            double x2 = _centerX + (_tickRadius - length) * Math.Cos(radians);
            double y2 = _centerY + (_tickRadius - length) * Math.Sin(radians);
        ...
        private void _DrawTicks()
        {
            ...
            double totalAngle = _maxAngle - _minAngle;

            for (int i = 0; i < MajorTicks; i++)
            {
                double angle = _minAngle - i * totalAngle / (MajorTicks - 1);
                _DrawTick(angle, 8, 2, Brushes.White);
            }
            ...
            if (MinorTicks > 0)
            {
                ...
                for (int i = 0; i < totalTicks; i++)
                {
                    ...
                    _DrawTick(angle, 4, 1, Brushes.White);
                }
            }
        }
        ...
        private void _DrawLabels()
        {
            ...
            for (int i = 0; i < MajorTicks; i++)
            {
                ...
                double angle = _minAngle - i * (_maxAngle - _minAngle) / (MajorTicks - 1);
        ...
        private void _DrawSegment(double startValue, double endValue, Color color)
        {
            ...
            bool isLargeArc = Math.Abs(startAngle - endAngle) > 180;
        ...
        private void _DrawSegments()
        {
            ...
            if (RedZoneLow.HasValue)
            {
                ...
                if (YellowZoneLow.HasValue)
                    _DrawSegment(RedZoneLow.Value, YellowZoneLow.Value, Colors.Yellow);
            }
            ...
            if (RedZoneHigh.HasValue)
            {
                if (YellowZoneHigh.HasValue)
                    _DrawSegment(YellowZoneHigh.Value, RedZoneHigh.Value, Colors.Yellow);
